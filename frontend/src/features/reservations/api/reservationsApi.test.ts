...
describe('reservationsApi', () => {
  ...
  describe('createReservation', () => {
    ...
    it('throws ReservationClientError on 409 idempotency conflict', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ok: false,
        status: 409,
        json: () => Promise.resolve({
          code: 'IDEMPOTENCY_CONFLICT',
          message: 'Chave de idempotência reutilizada com payload incompatível.',
          traceId: 'tr-conflict',
          timestamp: '2026-08-16T14:00:00Z',
        }),
      }));
      ...
      await expect(
        createReservation('e1', 's1', { quantity: 2 }, 'custom-key-123'),
      ).rejects.toThrow(ReservationClientError);
    });

    it('preserves provided idempotencyKey across retries and sets header', async () => {
      const mockResponse: ReservationResponse = {
        id: '11111111-1111-1111-1111-111111111111',
        customerId: '22222222-2222-2222-2222-222222222222',
        eventId: '33333333-3333-3333-3333-333333333333',
      ...
      await createReservation('e1', 's1', { quantity: 2 }, 'preserved-attempt-key');

      const headers = fetchMock.mock.calls[0][1].headers as Headers;
      expect(headers.get('Idempotency-Key')).toBe('preserved-attempt-key');
    });
  });
});
